...
export function ContactForm({ embedded = false }: { embedded?: boolean }) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setStatus("error")
      setError("Something went wrong. Try again or email shaan@technestsolutions.in")
    }
  }

  const inputCls = "w-full rounded-lg border border-zinc-700 bg-zinc-800/80 px-3.5 py-2.5 text-sm text-white placeholder:text-zinc-500 focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary/60 transition-colors"
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="flex flex-col gap-1.5">
              <label htmlFor="cf-name" className="text-xs font-medium text-zinc-400">Name</label>
              <input id="cf-name" name="name" type="text" required placeholder="Your name" className={inputCls} />
            </div>
            <div className="flex flex-col gap-1.5">
              <label htmlFor="cf-email" className="text-xs font-medium text-zinc-400">Work email</label>
              <input id="cf-email" name="email" type="email" required placeholder="you@company.com" className={inputCls} />
            </div>
          </div>

          <div className="flex flex-col gap-1.5">
            <label htmlFor="cf-type" className="text-xs font-medium text-zinc-400">What do you need?</label>
            <select
              id="cf-type"
              name="projectType"
              required
          ...
          <div className="flex flex-col gap-1.5">
            <label htmlFor="cf-message" className="text-xs font-medium text-zinc-400">Brief description</label>
            <textarea
              id="cf-message"
              name="message"
              required
          ...
          >
            {status === "loading"
              ...
              : <><span>Send message</span><RiArrowRightLine size={15} /></>
            }
          </button>
